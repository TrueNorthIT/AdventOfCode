...
from JoesAOCHelpers import Grid, Coord, extract_ints_from_string
...
class Day09Solver(JoesAoCSolver):
    ...
    def part1(self):
        ...
        iterations = map(
            lambda pair: self.area(*pair), list(itertools.combinations(red_tiles, 2))
        )
...
7,3""",
    ...
    def area_p2(
        self, p1: Coord, p2: Coord, polygon: list[Coord], perimiter_set: set[Coord]
    ) -> int:
    ...
    def line_crosses_polygon(
        self, pA: Coord, pB: Coord, perimiter_set: set[Coord]
    ) -> bool:
        ...
        if pA.x == pB.x:
            ...
            for y in range(min(pA.y, pB.y), max(pA.y, pB.y) + 1):
                if Coord(pA.x, y) in perimiter_set:
                    print(
                        f"Line from {pA} to {pB} crosses perimeter at {Coord(pA.x, y)}"
                    )
                    return True
        elif pA.y == pB.y:
            # Horizontal line
            for x in range(min(pA.x, pB.x), max(pA.x, pB.x) + 1):
                if Coord(x, pA.y) in perimiter_set:
                    print(
                        f"Line from {pA} to {pB} crosses perimeter at {Coord(x, pA.y)}"
                    )
                    return True
        else:
            ...
            for x in range(min(pA.x, pB.x), max(pA.x, pB.x) + 1):
                for y in range(min(pA.y, pB.y), max(pA.y, pB.y) + 1):
                    if Coord(x, y) in perimiter_set:
                        print(
                            f"Line from {pA} to {pB} crosses perimeter at {Coord(x, y)}"
                        )
                        return True
    ...
    def rectangle_inside_polygon(
        self, rect: list[Coord], poly: list[Coord], perimiter_set: set[Coord]
    ) -> bool:
    ...
    def part2(self):
        ...
        for i, (pair, simple) in enumerate(candidates_and_simple_areas):
            ...
            area = self.area_p2(*pair, red_tiles, perimiter_set)
            if area != None:
