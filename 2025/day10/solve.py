...
class Day10Solver(JoesAoCSolver):
    ...
    def solve_machine_lights(self, lights: list[int], buttons: list[tuple[int]]) -> list[int]:
        ...
        for combo in combos:
          ...
          for buttons_to_press in combo:
            ...
            if light_state == lights:
              print(f"Machine solved with {len(combo)} button presses: {combo}")
              return len(combo)
    ...
    def configure_machine_joltage_bfs(self, buttons: list[tuple[int, ...]], joltage: list[int]) -> int:
      ...
      while queue:
        ...
        if explored_states % LOG_EVERY == 0:
          print(f"  Visited {explored_states:,} states so far (depth={presses})...")
