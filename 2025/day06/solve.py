...
class Day06Solver(JoesAoCSolver):
    ...
    def parse_input(self):
      ...
      for line in p2_padded[:-1]:
        ...
        for col in range(question_len):
          digits = [p1[index] for index in range(col, len(p1)-question_len, question_len)]
    ...
    def part1(self):
      ...
      for i, op in enumerate(questions):
        digits = [int(worksheet[index]) for index in range(i, len(worksheet)-question_len, question_len)]
...
*   +   *   +  """, 4277556)]
    ...
    def part2(self):
      ...
      total = 0
      for i, op in enumerate(questions):
        digits = [worksheet[index] for index in range(i, len(p1)-question_len, question_len)]
        ...
        for col in range(col_len):
          new_number_str = "".join(new_digits[row][col] for row in range(len(new_digits)) if new_digits[row][col] != ".")
