...
public class PuzzleManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (var (boxAPos, boxBPos, distance) in distances)
        {
            ...
            List<Transform> isolatedBoxes = new List<Transform>();
            ...
            foreach (var obj in allObjects)
            {
                ...
                else if (obj.name.StartsWith("Cube") && obj.transform.parent == null)
                {
                    isolatedBoxes.Add(obj.transform);
                }
            }
            
            if (circuitGroups.Count == 1 && isolatedBoxes.Count == 0)
            {
                Debug.Log("All junction boxes connected into a single group.");
                float distanceFromWall = boxA.transform.position.x * boxB.transform.position.x;
                Debug.Log($"boxA x-position: {boxA.transform.position.x}, boxB x-position: {boxB.transform.position.x}");
                Debug.Log($"Distance of last connection from wall: {distanceFromWall}");
                break;
            }
        }
    ...
    void GetLargestGroups()
    {
        ...
        for (int i = 0; i < Mathf.Min(circuitGroups.Count); i++)
        {
